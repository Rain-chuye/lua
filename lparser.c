  ...
  if (ls->t.token != ')') {  /* is 'parlist' not empty? */
    do {
      switch (ls->t.token) {
        ...
        default: luaX_syntaxerror(ls, "期望 <name> 或 '...' ");
      }
...
static void expr (LexState *ls, expdesc *v) {
  ...
  if (testnext(ls, '?')) {
    ...
    int base = fs->freereg;
    luaK_exp2nextreg(fs, v);
    jf = luaK_codeAsBx(fs, OP_TERNARY, v->u.info, NO_JUMP);
    fs->freereg = base;
    expr(ls, &e1);
    luaK_exp2nextreg(fs, &e1);
    checknext(ls, ':');
    int jend = luaK_jump(fs);
    luaK_patchtohere(fs, jf);
    fs->freereg = base;
    expr(ls, &e2);
    luaK_exp2nextreg(fs, &e2);
    luaK_patchtohere(fs, jend);
    v->k = VNONRELOC;
    v->u.info = base;
    fs->freereg = base + 1;
  }
...
/*
** compiles the 主函数, which is a regular vararg function with an
...
static void mainfunc (LexState *ls, FuncState *fs) {
  ...
  expdesc v;
  open_func(ls, fs, &bl);
  fs->f->is_vararg = 2;  /* 主函数 is always declared vararg */
