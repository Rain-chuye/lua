...
static int push_captures (MatchState *ms, const char *s, const char *e) {
  ...
  luaL_checkstack(ms->L, nlevels, "捕获过多");
  for (i = 0; i < nlevels; i++)
  ...
  for (i = 0; i < l; i++) {
    if (news[i] != L_ESC)
      ...
      if (!isdigit(uchar(news[i]))) {
        if (news[i] != L_ESC)
          luaL_error(L, "替换字符串中无效使用了 '%c'", L_ESC);
        luaL_addchar(b, news[i]);
      }
      else if (news[i] == '0')
...
static void add_value (MatchState *ms, luaL_Buffer *b, const char *s,
  ...
  if (!lua_toboolean(L, -1)) {  /* nil or false? */
    ...
    luaL_error(L, "无效的替换值 (%s)", luaL_typename(L, -1));
    ...
    if (*strfrmt != L_ESC)
      ...
      switch (*strfrmt++) {
        ...
        default: {  /* also treat cases 'pnLlh' */
          return luaL_error(L, "给 'format' 传递了无效选项 '%%%c'",
                               *(strfrmt - 1));
        }
      }
...
static KOption getoption (Header *h, const char **fmt, int *size) {
  ...
  *size = 0;  /* default */
  switch (opt) {
    ...
    default: luaL_error(h->L, "无效格式选项 '%c'", opt);
  }
  return Knop;
}
...
static KOption getdetails (Header *h, size_t totalsize,
  ...
  if (opt == Kpaddalign) {  /* 'X' gets alignment from following option */
    if (**fmt == '\0' || getoption(h, fmt, &align) == Kchar || align == 0)
      luaL_argerror(h->L, 1, "无效 next option for option 'X'");
  }
...
static int str_unpack (lua_State *L) {
  ...
  while (*fmt != '\0') {
    ...
    luaL_checkstack(L, 2, "过多 results");
    n++;
    switch (opt) {
      case Kint:
      case Kuint: {
