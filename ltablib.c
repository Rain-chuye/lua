...
static int tinsert (lua_State *L) {
  ...
  switch (lua_gettop(L)) {
    ...
    case 3: {
      ...
      luaL_argcheck(L, 1 <= pos && pos <= e, 2, "position 越界");
    ...
    default: {
      return luaL_error(L, "'insert' 的参数个数错误");
    }
  }
...
static int tremove (lua_State *L) {
  ...
  if (pos != size)  /* validate 'pos' if given */
    luaL_argcheck(L, 1 <= pos && pos <= size + 1, 1, "position 越界");
...
static void addfield (lua_State *L, luaL_Buffer *b, lua_Integer i) {
  lua_geti(L, 1, i);
  if (!lua_isstring(L, -1))
    luaL_error(L, "无效 value (%s) at index %d in table for 'concat'",
                  luaL_typename(L, -1), i);
  luaL_addvalue(b);
}
...
static int unpack (lua_State *L) {
  ...
  if (n >= (unsigned int)INT_MAX  || !lua_checkstack(L, (int)(++n)))
    return luaL_error(L, "unpack 的结果过多");
