      ...
      vmcase(OP_FORPREP) {
        ...
        if (ttisinteger(init) && ttisinteger(pstep) &&
          ...
          if (!tonumber(plimit, &nlimit))
            luaG_runerror(L, "'for' 限制必须是数字");
          setfltvalue(plimit, nlimit);
          if (!tonumber(pstep, &nstep))
            luaG_runerror(L, "'for' 步长必须是数字");
          setfltvalue(pstep, nstep);
          if (!tonumber(init, &ninit))
            luaG_runerror(L, "'for' 初始值必须是数字");
        ...
        vmcase(OP_TFOREACH) {
          ...
          lua_pushcfunction(L, l_next);  /* will return generator, */
          lua_pushvalue(L,-2);
